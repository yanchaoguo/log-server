	...
	private Request(ChannelHandlerContext ctx, FullHttpRequest nettyRequest) {
		...
		if(nettyRequest.getMethod() != HttpMethod.GET){
			...
			try {
				...
				System.out.println("body中的内容无法解析为参数:"+content);
			}
			finally {
				if(null != decoder){
					decoder.destroy();
		...
		// IP
		this.putIp(ctx);
//		this.ip = getIp(nettyRequest ,ctx);
	}
	...
	public boolean isKeepAlive() {
		final String connectionHeader = getHeader(HttpHeaders.Names.CONNECTION.toString());
		// 无论任何版本Connection为close时都关闭连接
		if (HttpHeaders.Values.CLOSE.toString().equalsIgnoreCase(connectionHeader)) {
			return false;
		}

		// HTTP/1.0只有Connection为Keep-Alive时才会保持连接
		if (HttpVersion.HTTP_1_0.text().equals(getProtocolVersion())) {
			if (false == HttpHeaders.Values.KEEP_ALIVE.toString().equalsIgnoreCase(connectionHeader)) {
				return false;
			}
		}
...
//		this.headers.get("Cookie");
	...
	protected void putIp(ChannelHandlerContext ctx) {
        ...
        String ra = ((InetSocketAddress) (ctx.channel().remoteAddress())).getAddress().getHostAddress();

        if(StrUtil.isNotEmpty(xff) && !"unKnown".equalsIgnoreCase(xff)){
            //多次反向代理后会有多个ip值，第一个ip才是真实ip
            ip = NetUtil.getMultistageReverseProxyIp(xff);
        }else if(StrUtil.isNotEmpty(xri) && !"unknown".equalsIgnoreCase(xri)){
            ip = xri;
        }else{
            ip = ra;
        }
