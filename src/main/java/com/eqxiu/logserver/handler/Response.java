...
import com.eqxiu.logserver.listener.FileProgressiveFutureListener;
...
public class Response {
	...
	public Response setContentLength(long contentLength) {
		setHeader(HttpHeaders.Names.CONTENT_LENGTH.toString(), contentLength);
		return this;
	}
	...
	public Response setKeepAlive() {
		setHeader(HttpHeaders.Names.CONNECTION.toString(), HttpHeaders.Values.KEEP_ALIVE.toString());
		return this;
	}
	...
	public void setDateAndCache(long lastModify, int httpCacheSeconds) {
		...
		setHeader(HttpHeaders.Names.DATE.toString(), formatter.format(time.getTime()));

		// Add cache headers
		time.add(Calendar.SECOND, httpCacheSeconds);

		setHeader(HttpHeaders.Names.EXPIRES.toString(), formatter.format(time.getTime()));
		setHeader(HttpHeaders.Names.CACHE_CONTROL.toString(), "private, max-age=" + httpCacheSeconds);
		setHeader(HttpHeaders.Names.LAST_MODIFIED.toString(), formatter.format(DateUtil.date(lastModify)));
	}
	...
	private DefaultHttpResponse toDefaultHttpResponse() {
		...
		for (Cookie cookie : cookies) {
			httpHeaders.add(HttpHeaders.Names.SET_COOKIE.toString(), ServerCookieEncoder.LAX.encode(cookie));
		}
	...
	private FullHttpResponse toFullHttpResponse() {
		...
		httpHeaders.set(HttpHeaders.Names.CONTENT_TYPE.toString(), StrUtil.format("{};charset={}", contentType, charset));
		httpHeaders.set(HttpHeaders.Names.CONTENT_ENCODING.toString(), charset);
		httpHeaders.set(HttpHeaders.Names.CONTENT_LENGTH.toString(), byteBuf.readableBytes());
	...
	public ChannelFuture sendRedirect(String uri) {
		return this.setStatus(HttpResponseStatus.FOUND).setHeader(HttpHeaders.Names.LOCATION.toString(), uri).send();
	}
	...
	public ChannelFuture sendNotModified() {
		return this.setStatus(HttpResponseStatus.NOT_MODIFIED).setHeader(HttpHeaders.Names.DATE.toString(), DateUtil.formatHttpDate(DateUtil.date())).send();
	}
